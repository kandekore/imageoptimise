...
const MAX_PER_FILE = 25 * 1024 * 1024; // mirrors backend default

function isImage(file) {
  return file.type.startsWith('image/');
}

export default function UploadStep({ files, setFiles, onBack, onNext }) {
  ...
  const addFiles = useCallback(
    (incoming) => {
      ...
      for (const f of incoming) {
        if (!isImage(f)) rejects.push({ name: f.name, reason: 'Not an image' });
        else if (f.size > MAX_PER_FILE) rejects.push({ name: f.name, reason: 'Exceeds 25MB' });
        else kept.push(f);
      }
      setFiles((prev) => {
  ...
  return (
    <>
      ...
      <p style={{ color: 'var(--text-dim)', marginTop: 0 }}>
        Drag & drop images or click to browse. JPG, PNG, WebP, HEIC, TIFF and GIF are accepted.
      </p>
      ...
      {rejected.length > 0 && (
        <div className="banner error" style={{ marginTop: '1rem' }}>
          Skipped {rejected.length} file(s): {rejected.map((r) => `${r.name} (${r.reason})`).join(', ')}
        </div>
      )}
